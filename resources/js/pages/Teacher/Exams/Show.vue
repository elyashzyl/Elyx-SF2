<template>
    ...
    <Link href="/teacher/exams" class="mb-6 inline-flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-[var(--gl-text-primary)]" style="color: var(--gl-text-secondary)">
        ...
        <div class="relative z-10 flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <div class="flex flex-wrap items-center gap-2 mb-1">
                        <h1 class="text-xl font-bold" style="color: var(--gl-text-primary)">{{ exam.title }}</h1>
                    ...
                    <p class="text-sm" style="color: var(--gl-text-secondary)">
                        ...
                        · {{ exam.sections.length }} sections · {{ exam.max_score }} points
                        <template v-if="exam.closes_at">
                            · <span :style="{ color: isClosed ? 'var(--gl-danger)' : 'var(--gl-success)' }">{{ isClosed ? 'Closed' : 'Closes ' + timeRemaining(exam.closes_at) }}</span>
                        </template>
                    </p>
                    <p v-if="exam.instructions" class="mt-2 text-sm" style="color: var(--gl-text-muted)">{{ exam.instructions }}</p>
                </div>
            </div>
            <div class="flex flex-wrap items-center gap-1.5">
                <button v-if="!isClosed" @click="togglePublish" class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors" style="background: var(--gl-surface-2); color: var(--gl-text-primary); border: 1px solid var(--gl-border);">
                    <UploadCloud class="h-3.5 w-3.5" :stroke-width="2" /> {{ exam.is_published ? 'Unpublish' : 'Publish' }}
                </button>
                <button v-if="exam.is_published && !isClosed" @click="closeNow" class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors" style="color: var(--gl-danger); border: 1px solid rgba(239,68,68,0.2); background: var(--gl-danger-bg);">
