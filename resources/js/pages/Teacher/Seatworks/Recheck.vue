<template>
    ...
    <Link :href="`/teacher/seatworks/${seatwork.id}`" class="mb-6 inline-flex items-center gap-1.5 text-sm font-medium hover:text-[#2B3444]" style="color: #5A6376">
    ...
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h2 class="text-lg font-semibold" style="color: #1B2231">Recheck: {{ seatwork.title }}</h2>
    ...
    <form @submit.prevent="save">
        <div v-for="(q, idx) in questions" :key="q.id" class="card mb-4 p-5">
            <div class="mb-2 flex items-center gap-2">
                <span class="text-xs font-semibold uppercase tracking-wider" style="color: #7C8598">{{ idx + 1 }}. {{ typeLabel(q.type) }}</span>
                <span class="text-xs" style="color: #5A6376">{{ q.points }} pt{{ q.points !== 1 ? 's' : '' }}</span>
            </div>
            <p class="mb-3 text-sm font-medium" style="color: #1B2231">{{ q.question_text }}</p>

            <!-- Identification -->
            <div v-if="q.type === 'identification'" class="mb-3 space-y-1 text-sm">
                <div class="rounded-lg bg-[#F5F6F8] px-4 py-2" style="color: #5A6376">
                    <span class="text-xs font-medium" style="color: #7C8598">Student's answer:</span>
                    <p>{{ studentAnswerText(q) || '—' }}</p>
                </div>
                <div class="rounded-lg bg-[#DCEEE3] px-4 py-2 text-sm" style="color: #2F7A54">
                    <span class="text-xs font-medium">Correct answer:</span>
                    <p>{{ q.options?.[0]?.option_text ?? '—' }}</p>
                </div>
            </div>

            <!-- Enumeration -->
            <div v-if="q.type === 'enumeration'" class="mb-3 space-y-1 text-sm">
                <div class="rounded-lg bg-[#F5F6F8] px-4 py-2" style="color: #5A6376">
                    <span class="text-xs font-medium" style="color: #7C8598">Student's answers:</span>
                    <div v-for="(line, li) in (studentAnswerText(q) || '').split('\n').filter(Boolean)" :key="li" class="flex items-center gap-2">
                        <span class="text-xs" style="color: #7C8598">{{ li + 1 }}.</span> {{ line }}
                    </div>
                    <p v-if="!studentAnswerText(q)" class="text-xs" style="color: #AEB4C0">No answer</p>
                </div>
                <div class="rounded-lg bg-[#DCEEE3] px-4 py-2 text-sm" style="color: #2F7A54">
                    <span class="text-xs font-medium">Correct answers:</span>
                    <div v-for="(opt, oi) in q.options" :key="oi" class="flex items-center gap-2">
                        <span class="text-xs" style="color: #2F7A54">{{ oi + 1 }}.</span> {{ opt.option_text }}
                    </div>
                </div>
            </div>

            <!-- True/False & Multiple Choice -->
            <div v-if="q.type === 'true_false' || q.type === 'multiple_choice'" class="mb-3 space-y-2">
            ...
            <!-- Matching -->
            <div v-if="q.type === 'matching'" class="mb-3 space-y-1 text-sm">
                <div v-for="pair in q.matching_pairs" :key="pair.id" class="flex items-center gap-2 rounded-lg bg-[#F5F6F8] px-4 py-2" style="color: #5A6376">
                    <span class="font-medium" style="color: #1B2231">{{ pair.left_text }}</span>
                    <ArrowRight class="h-3.5 w-3.5 shrink-0" :stroke-width="2" style="color: #AEB4C0" />
                    <span class="text-[#2F7A54]">{{ pair.right_text }}</span>
                    <span class="ml-auto text-xs" style="color: #7C8598">→ {{ studentMatchingAnswer(q, pair) }}</span>
                </div>
            </div>

            <!-- Toggle correct/incorrect -->
            <div class="flex items-center gap-2 border-t border-[#E9EBEF] pt-3">
...
import { ArrowLeft, Check, RotateCw, X, ArrowRight, LoaderCircle } from '@lucide/vue';
import { computed, ref } from 'vue';

const props = defineProps<{ seatwork: any; attempt: any }>();

const totalPoints = computed(() => props.seatwork.questions.reduce((s: number, q: any) => s + q.points, 0));
...
const questions = computed(() => {
    return props.seatwork.questions.map((q: any) => {
...
function studentAnswerText(q: any): string {
    return q.studentAnswer?.answer_text ?? '';
}

function studentMatchingAnswer(q: any, pair: any): string {
    const ma = q.studentAnswer?.matching_answers;
    if (!ma) return '—';
    return ma[pair.id] ?? '—';
}

function isSelected(opt: any, question: any): boolean {
    return question.studentAnswer?.selected_option_id === opt.id;
}
...
function autoRecheck() {
    answerData.value.forEach((d: AnswerDatum) => {
        ...
        if (!ans) return;
        const question = props.seatwork.questions.find((q: any) => q.id === ans.question_id);
        if (!question) return;

        switch (question.type) {
            case 'identification': {
                const correct = question.options?.[0]?.option_text ?? '';
                d.is_correct = ans.answer_text?.trim().toLowerCase() === correct.trim().toLowerCase();
                break;
            }
            case 'enumeration':
                d.is_correct = false;
            ...
            case 'multiple_choice': {
                ...
                d.is_correct = correctOption ? ans.selected_option_id === correctOption.id : false;
                break;
            }
            case 'matching': {
                const ma = ans.matching_answers ?? {};
                const pairs = question.matching_pairs ?? [];
                d.is_correct = pairs.length > 0 && pairs.every((p: any) => ma[p.id] === p.right_text);
                break;
            }
            default:
                d.is_correct = false;
...
function save() {
    saving.value = true;
    router.put(`/teacher/seatworks/${props.seatwork.id}/recheck/${props.attempt.id}`, {
