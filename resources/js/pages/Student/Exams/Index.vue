    ...
    <div class="relative mb-8 overflow-hidden rounded-2xl p-8 gl-fade-in"
        ...
        <div class="relative z-10">
            ...
            <p class="mt-2 max-w-lg text-sm" style="color: var(--gl-text-secondary)">
                Complete exams, earn XP, unlock achievements, and climb the leaderboard.
            </p>
        </div>
        ...
        <div class="min-w-0 flex-1">
            <div v-if="!exams.length" class="gl-glow-card flex flex-col items-center justify-center px-8 py-16 text-center">
                <div class="mb-6 flex h-20 w-20 items-center justify-center rounded-2xl"
                    ...
                    <FileText class="h-10 w-10" style="color: var(--gl-secondary);" :stroke-width="1.5" />
                </div>
                <h3 class="text-lg font-semibold" style="color: var(--gl-text-primary)">No Exams Available Yet</h3>
                <p class="mt-2 max-w-md text-sm" style="color: var(--gl-text-secondary)">
                    Your teacher hasn't published any exams yet. Check back later to continue earning XP.
                </p>
                <div class="mt-6 flex max-w-sm items-center gap-3 rounded-xl p-4"
                    ...
                    <p class="text-xs text-left" style="color: var(--gl-text-secondary)">
                        Complete exams to earn XP, unlock badges, increase your level, and collect rewards.
                    </p>
                </div>
            </div>
            ...
            <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div v-for="exam in exams" :key="exam.id"
                    ...
                    :class="{ 'opacity-60': exam.status === 'submitted' }">
                    <div class="mb-3 flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl"
                                :style="{ background: statusGradient(exam.status) }">
                                <component :is="statusIcon(exam.status)" class="h-5 w-5 text-white" :stroke-width="2" />
                            </div>
                            <div class="min-w-0">
                                <h3 class="text-sm font-semibold truncate" style="color: var(--gl-text-primary)">{{ exam.title }}</h3>
                                <p v-if="exam.instructions" class="text-xs truncate mt-0.5" style="color: var(--gl-text-muted)">{{ exam.instructions }}</p>
                            </div>
                        </div>
                        <span class="shrink-0 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium"
                            :style="{ background: statusBadgeBg(exam.status), color: statusBadgeText(exam.status) }">
                            <component :is="statusBadgeIcon(exam.status)" class="h-3 w-3" :stroke-width="2" />
                            {{ statusLabel(exam.status) }}
                        </span>
                    </div>
                    <div class="mb-4 flex flex-wrap gap-x-4 gap-y-1.5 text-xs" style="color: var(--gl-text-muted)">
                        <span class="flex items-center gap-1.5">
                            <Layers class="h-3.5 w-3.5" :stroke-width="2" style="color: var(--gl-secondary);" />
                            {{ exam.sections_count }} sections
                        </span>
                        <span v-if="exam.time_limit_minutes" class="flex items-center gap-1.5">
                            <Clock class="h-3.5 w-3.5" :stroke-width="2" style="color: var(--gl-primary);" />
                    ...
                    <div v-if="exam.status === 'submitted'" class="mb-4">
                        <div class="flex items-center gap-3">
                            <div class="gl-xp-bar flex-1">
                                <div class="gl-xp-bar-fill" :style="{ width: scorePct(exam) + '%' }"></div>
                            </div>
                    ...
                    <div class="mt-auto">
                        <Link v-if="exam.status !== 'submitted'"
                        ...
                        <Link v-else
                            :href="`/student/exams/${exam.attempt_id}/result`"
            ...
            <div class="sticky space-y-4" style="top: 80px;">
                <div class="gl-glow-card p-5">
                    <div class="mb-3 flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold" style="color: var(--gl-text-primary)">Exam Mastery</p>
                ...
                <div class="gl-glow-card p-5">
                    ...
                    <div class="space-y-2.5">
                        <div class="flex items-center gap-2.5 text-xs">
                            ...
                            <span style="color: var(--gl-text-secondary)">Complete exams to level up</span>
                        </div>
                        <div class="flex items-center gap-2.5 text-xs">
                            <Award class="h-4 w-4 shrink-0" style="color: var(--gl-secondary);" :stroke-width="2" />
                            <span style="color: var(--gl-text-secondary)">Earn badges for perfect scores</span>
                        </div>
                        <div class="flex items-center gap-2.5 text-xs">
                            <Zap class="h-4 w-4 shrink-0" style="color: var(--gl-primary);" :stroke-width="2" />
