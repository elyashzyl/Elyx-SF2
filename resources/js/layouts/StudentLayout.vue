...
async function checkMessengerStatus() {
    try {
        ...
        messengerSystemEnabled.value = true;
    }
}

async function fetchMessengerUnread() {
    if (!messengerSystemEnabled.value || user.messenger_enabled == false) {
...
const nav = computed(() => [
    {
        ...
        active: path.value === '/student/dashboard',
    },
    { label: 'LEARNING' },
    {
        name: 'Quizzes',
        ...
        icon: FileQuestion,
        active:
            path.value.startsWith('/student/quizzes') &&
            !path.value.includes('/take') &&
            !path.value.includes('/result'),
    },
    {
        name: 'Exams',
        href: '/student/exams',
        icon: FileText,
        active:
            path.value.startsWith('/student/exams') &&
            !path.value.includes('/take') &&
            !path.value.includes('/result'),
    },
    {
        name: 'Seatworks',
        href: '/student/seatworks',
        icon: ClipboardList,
        active:
            path.value.startsWith('/student/seatworks') &&
            !path.value.includes('/take') &&
            !path.value.includes('/result'),
    },
    {
        ...
        icon: FlaskConical,
        active:
            path.value.startsWith('/student/practicals') &&
            !path.value.includes('/take') &&
            !path.value.includes('/result'),
    },
    { label: 'PRACTICE' },
    {
        ...
        href: '/student/games',
        icon: Gamepad2,
        active: path.value.startsWith('/student/games'),
    },
    { label: 'PROGRESS' },
    {
        name: 'My Results',
        ...
        icon: ClipboardCheck,
        active: path.value.startsWith('/student/results'),
    },
    {
        name: 'Leaderboard',
        href: '/student/leaderboard',
        icon: Trophy,
        active: path.value.startsWith('/student/leaderboard'),
    },
    { label: 'CONNECT' },
]);
...
const navFinal = computed(() => {
    ...
    if (messengerSystemEnabled.value) {
        items.push({
            name: 'Messenger',
            href: '/student/messenger',
            icon: MessageCircle,
            active: path.value.startsWith('/student/messenger'),
        });
    }
...
<template>
    <div
        ...
        style="background: var(--gl-bg); color: var(--gl-text-primary)"
    >
        <aside
        ...
        >
            <div
                class="relative flex h-16 items-center gap-3 overflow-hidden border-b px-5"
            ...
            >
                ...
                ></div>
                <div
                    class="relative z-10 flex h-10 w-10 items-center justify-center rounded-xl"
                    style="
                        ...
                        box-shadow: 0 0 20px var(--gl-primary-glow);
                    "
                >
                    <GraduationCap
            ...
            <nav
                class="messenger-scroll flex-1 space-y-1 overflow-y-auto px-3 py-3"
            >
                <template v-for="link in navFinal" :key="link.name">
                    <div
                        v-if="link.label"
                        class="px-3 pt-3 pb-1 text-[10px] font-semibold tracking-wider uppercase"
                    ...
                    >
                        <span
                            v-if="link.active"
                            class="absolute top-1/2 left-0 h-5 w-0.5 -translate-y-1/2 rounded-r-full"
                            style="
                                background: var(--gl-primary);
                                box-shadow: 0 0 6px var(--gl-primary-glow);
                            "
                        ></span>
                        <component
                            :is="link.icon"
                        ...
                        <span
                            v-if="
                                link.name === 'Messenger' && messengerUnread > 0
                            "
                            class="flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] leading-none font-bold text-white"
                            style="background: var(--gl-danger)"
                            >{{ messengerUnread }}</span
                        >
                    </Link>
                </template>
            </nav>
            <div class="p-3" style="border-top: 1px solid var(--gl-border)">
                ...
                >
                    ...
                    >
                        ...
                        <button
                            @click="logout"
                            class="shrink-0 rounded-lg p-1.5 transition-all hover:bg-[rgba(239,68,68,0.1)] hover:text-[var(--gl-danger)]"
                            style="color: var(--gl-text-muted)"
                            title="Sign out"
                        >
        ...
        <div class="flex min-h-screen flex-1 flex-col" style="min-width: 0">
            <header
                class="sticky top-0 z-20 flex h-16 items-center gap-4 px-6"
                :style="{
            ...
            >
                ...
                >
                    <UserCheck class="h-4 w-4" :stroke-width="2" /><span
                        class="font-medium"
                        >Impersonating {{ user.name }}</span
                    ><button
                        @click="stopImpersonating"
                        class="ml-auto rounded-lg px-3 py-1 text-xs font-semibold"
                        style="background: var(--gl-accent); color: #0f172a"
                    >
                        Stop
                    </button>
                </div>
                <div v-else class="flex w-full items-center gap-4">
                    ...
                    <ThemeToggle /><NotificationBell role="student" />
                </div>
            </header>
            <main
            ...
            >
                <div
                    class="mr-auto flex min-h-0 flex-1 flex-col"
