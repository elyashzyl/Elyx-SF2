...
function publicUser(row) {
  ...
  const school = userData.school_id ? schoolToResponse(getSchoolById(userData.school_id)) : null
...
router.post('/impersonate', (req, res) => {
  ...
  const target = query('SELECT * FROM users WHERE id = ?', [targetId])[0]
  ...
  res.json({ user: publicUser(target), impersonatedBy: { id: me.id, name: me.name } })
})
...
router.post('/impersonate/stop', (req, res) => {
  ...
  const admin = query('SELECT * FROM users WHERE id = ?', [superadminId])[0]
  if (!admin || admin.role !== 'superadmin') {
    return res.status(403).json({ error: 'Original superadmin session is no longer valid' })
  }
  const stopped = userId ? query('SELECT * FROM users WHERE id = ?', [userId])[0] : null
  logAudit({
