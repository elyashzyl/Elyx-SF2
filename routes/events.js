...
router.get('/calendar', async (req, res) => {
  ...
  try {
    const events = s.schoolId
      ? await query('SELECT * FROM calendar_events WHERE school_id = ? ORDER BY event_date', [s.schoolId])
      : await query('SELECT * FROM calendar_events ORDER BY event_date')
    res.json(events)
  } catch {
...
router.put('/calendar/:id', async (req, res) => {
  ...
  if (!s) return
  try {
    const chk = await ownRow('calendar_events', req.params.id, s.schoolId, s.me)
    ...
    const { title, type, event_date, color } = req.body
    await run('UPDATE calendar_events SET title=?, type=?, event_date=?, color=? WHERE id=?',
...
router.delete('/calendar/:id', async (req, res) => {
  ...
  try {
    ...
    if (!chk.row) return res.status(404).json({ error: 'Event not found' })
    if (chk.denied) return res.status(403).json({ error: 'Forbidden: outside your school' })
    await run('DELETE FROM calendar_events WHERE id=?', [req.params.id])
    res.json({ success: true })
  } catch {
...
router.post('/quarterly', async (req, res) => {
  ...
  try {
    ...
    await run('INSERT INTO quarterly_events (event_name, first_grading, second_grading, third_grading, fourth_grading, school_id) VALUES (?, ?, ?, ?, ?, ?)',
...
router.put('/quarterly/:id', async (req, res) => {
  const s = await writable(req, res)
  if (!s) return
  try {
    ...
    if (chk.denied) return res.status(403).json({ error: 'Forbidden: outside your school' })
    const { event_name, first_grading, second_grading, third_grading, fourth_grading } = req.body
    await run('UPDATE quarterly_events SET event_name=?, first_grading=?, second_grading=?, third_grading=?, fourth_grading=? WHERE id=?',
...
router.delete('/quarterly/:id', async (req, res) => {
  const s = await writable(req, res)
  if (!s) return
  try {
    const chk = await ownRow('quarterly_events', req.params.id, s.schoolId, s.me)
