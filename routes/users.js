...
router.get('/', (req, res) => {
  ...
  const users = scope.schoolId
    ? query('SELECT id, username, name, role, grade, section, period, school_id FROM users WHERE school_id = ? OR (school_id IS NULL OR school_id = ?) AND role = ? ORDER BY name', [scope.schoolId, '', 'superadmin'])
    : query('SELECT id, username, name, role, grade, section, period, school_id FROM users ORDER BY name')
  res.json(users)
})
...
router.post('/', (req, res) => {
  ...
  if (role === 'teacher' && !assertValidClass(res, targetSchoolId, grade, section)) return
  const id = uuidv4()
  ...
  audit(me, 'user.create', { type: 'user', id, name: `${name} (${username})`, schoolId: targetSchoolId || '' }, `Created ${role} "${username}"`)
...
router.put('/:id', (req, res) => {
  ...
  if (error) return
  const { id } = req.params
  const target = query('SELECT * FROM users WHERE id = ?', [id])[0]
  ...
  const dup = query('SELECT id FROM users WHERE username = ? AND id != ?', [username || target.username, id])
  if (dup.length > 0) {
    return res.status(400).json({ error: 'Username already exists' })
  }
  ...
  if (newRole === 'teacher') {
    ...
    if (effSchool && !assertValidClass(res, effSchool, effGrade, effSection)) return
  }
  const vals = {
  ...
  audit(me, 'user.update', { type: 'user', id, name: `${vals.username}`, schoolId: newRole === 'superadmin' ? '' : (newSchoolId || '') }, `Updated user "${target.username}"`)
...
router.delete('/:id', (req, res) => {
  ...
  audit(me, 'user.delete', { type: 'user', id, name: `${target.name} (${target.username})`, schoolId: target.school_id || '' }, `Deleted ${target.role} "${target.username}"`)
  res.json({ success: true })
})
